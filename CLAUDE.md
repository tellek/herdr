...
The global menu (`src/app/input/modal.rs::global_menu_actions`, ordered labels in `src/app/input/sidebar.rs::global_menu_labels`) has a "headroom" item, right after "reload config". Selecting it sets `AppState::request_toggle_headroom_proxy`; the consuming loop (`App::mod.rs` run loop and `src/server/headless.rs`, mirroring the `request_reload_config` flag pattern) calls `App::toggle_headroom_proxy()` (`src/app/headroom.rs`), which spawns or kills a detached `headroom proxy --port 8787` child process (`headroom_proxy_child: Option<std::process::Child>` on `App`, session-scoped and not persisted — `AppState` stays pure data). `AppState::headroom_proxy_running` is the UI-visible flag; `global_menu_item_has_badge` shows an orange accent-colored dot next to "headroom" while it's set. `App::poll_headroom_proxy()` clears the flag if the child exits on its own (crash, killed externally). On Windows the child is spawned with `CREATE_NO_WINDOW` so no console flashes.
...
**Toggling off a proxy herdr didn't spawn.** If `headroom_proxy_running` is `true` but `headroom_proxy_child` is `None` (the port-probe case above — e.g. a proxy left running by a previous herdr process), `toggle_headroom_proxy` has no child handle to kill, so it falls back to `kill_process_on_port(HEADROOM_PROXY_PORT)` (`src/app/headroom.rs`): on Windows, runs `netstat -ano`, parses the `LISTENING` row for the port with `parse_windows_netstat_pid` (pure string parsing, unit-tested independent of the OS via `cfg(any(windows, test))`) to get the PID, then `taskkill /PID <pid> /F`; on Unix, `lsof -ti :<port>` + `kill -9`. All three helper `Command`s (netstat/taskkill/spawn) go through `no_console_window` on Windows so nothing flashes.
