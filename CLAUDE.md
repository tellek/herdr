...
**Nothing is typed except the resume command.** An earlier attempt at #13 prefixed the typed input with `Set-Location`/`cd` as insurance against a shell startup file moving the shell. That races with shell startup: bytes written to the PTY before the shell (PowerShell in particular, which herdr starts with `-NoExit -Command <wrapper>` for OSC 7 CWD reporting) finishes initializing can be partly swallowed, so the agent ran before the directory change landed and reported a missing session. `start_pending_agent_resume` (`src/app/agent_resume.rs`) sends only the resume command; the directory comes from `CommandBuilder::cwd`, which applies to the shell process before it starts and cannot race.
