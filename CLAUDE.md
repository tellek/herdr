...
**Git-repo-root fallback when `project_cwd` is absent (issue #5).** `agent_session_project_cwd` is only populated by a hook new enough to send `--project-cwd`/`project_cwd`; an outdated integration install (pre-v9 `herdr-agent-state` script) never sends it, so `PaneAgentSessionSnapshot.project_cwd` stays `None` for those sessions even after the issue #9 fix. In `persist/restore.rs`, when resuming an agent and no (existing-on-disk) `project_cwd` is stored, the resume CWD now falls back to `crate::workspace::git_repo_root(&cwd)` — walking up from the saved pane CWD to the nearest git repo root — before falling back further to the raw saved CWD. `git_repo_root` (in `src/workspace/git/discovery.rs`) was promoted from `pub(super)` to `pub` and re-exported through `workspace::git::mod.rs` and `workspace.rs` for this. This makes resume robust to stale/outdated integration hooks without requiring the user to reinstall them, and complements (not replaces) the `agent_session_project_cwd` mechanism above.
