...
**The resume command `cd`s explicitly.** Setting the spawn CWD is not sufficient on its own: a shell startup file, or the `-NoExit -Command` PowerShell wrapper herdr injects for OSC 7 CWD reporting, can move the shell before it reads herdr's input. `start_pending_agent_resume` (`src/app/agent_resume.rs`) therefore prefixes the resume command with a directory change built by `shell_cd_prefix`: `Set-Location -LiteralPath '<dir>'; ` for PowerShell (no `&&` before pwsh 7; `-LiteralPath` avoids glob expansion), `cd '<dir>' && ` otherwise. Shell selection reuses `crate::pane::pane_shell`, and PowerShell detection uses `crate::pane::is_powershell_shell` (both promoted to `pub(crate)`; `is_powershell_shell` replaced the `#[cfg(windows)]`-only `is_windows_powershell_shell`).
