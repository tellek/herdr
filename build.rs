...
fn main() {
    ...
    // When LIBGHOSTTY_VT_PREBUILT=true, skip the Zig build and use the artifacts already in
    // vendor/libghostty-vt/zig-out/lib/. This is useful on machines where Zig 0.15.2 is not
    // installed (e.g. a newer Zig version is present) but pre-built artifacts exist from a prior
    // build, so that `cargo clippy` / `just lint` still work without re-running the Zig build.
    let prebuilt = env_bool("LIBGHOSTTY_VT_PREBUILT").unwrap_or(false);
    if !prebuilt {
