...
**Resume uses the saved pane CWD (issue #13).** The #5/#9 precedence is inverted: restore spawns the resume shell in the saved pane CWD when it still exists, falls back to `project_cwd` only when it does not, and no longer walks up to the git repo root (`git_repo_root` is `pub(super)` again). The old order moved resumed panes out of the folder the user left them in, which the resumed agent then inherits as its CWD. `start_pending_agent_resume` also prefixes the resume command with an explicit directory change (`shell_cd_prefix`: `Set-Location -LiteralPath` for PowerShell, `cd ... &&` elsewhere) so a shell startup file or herdr's own PowerShell wrapper cannot move the shell first.
