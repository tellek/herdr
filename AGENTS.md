...
**Resume uses the saved pane CWD (issue #13).** The #5/#9 precedence is inverted: restore spawns the resume shell in the saved pane CWD when it still exists, falls back to `project_cwd` only when it does not, and no longer walks up to the git repo root (`git_repo_root` is `pub(super)` again). The old order moved resumed panes out of the folder the user left them in, which the resumed agent then inherits as its CWD. The directory comes solely from `CommandBuilder::cwd` on the spawned shell. Do not prefix the typed resume input with `cd`/`Set-Location`: input written before the shell finishes initializing can be partly swallowed, which launched the agent before the directory change landed.
