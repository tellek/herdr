...
## Panel tracks all terminal sessions, not just detected agents (issue #6)

`Tab::pane_details()` (`src/workspace/aggregate.rs`) used to `return None` for any pane whose terminal had neither an explicit `agent_name` nor a detected agent (`effective_agent_label()`), so a plain shell with no agent running never showed up in the sidebar. That filter is gone: every attached pane now yields a `PaneDetail`/`AgentPanelEntry`. `PaneDetail::agent_label` and `AgentPanelEntry::agent_label` changed from `String` to `Option<String>` — `Some(type)` when an agent is detected/named, `None` for a plain shell. `src/ui/sidebar.rs`'s status row already guarded the `· <agent_label>` segment on `Some`, so no rendering logic changed there. A plain shell reports `AgentState::Unknown`, which renders as the "idle" status with a dim dot, indistinguishable from an idle agent except for the missing `· <type>` suffix.
...
`encode_paste_payload` (`src/pane.rs`) normalizes `\r\n`/`\r` → `\n`, then wraps in `\x1b[200~…\x1b[201~` when the pane has bracketed paste enabled (`InputState::bracketed_paste`) or backslash-escapes newlines otherwise. On Windows the paste arrives one console event at a time, so `windows_stdin_reader_loop` (`src/client/input.rs`) reassembles the bracketed-paste byte stream in `RawInputFramer` while `raw_sequence_pending` is set. Pasted newlines/tabs surface as `KeyCode::Enter`/`KeyCode::Tab`; `windows_key_raw_bytes` returns raw `\r`/`\t` for those **only while a sequence is pending**, so they stay inside the paste instead of interrupting it and reaching the pane as a literal Enter (the bug where Claude submitted on the first pasted line). Outside a pending sequence Enter/Tab stay normal semantic keys. That reassembly only works because terminal setup enables `ENABLE_VIRTUAL_TERMINAL_INPUT` on the console input handle (`crate::platform::enable_console_vt_input`, restored on teardown); without it the console strips the `\x1b[200~`/`\x1b[201~` markers and a paste degrades to a bare burst of `Char`+`Enter` keys, so pasted newlines submit. crossterm 0.29 never sets this flag and never emits `Event::Paste` on Windows. As a belt-and-suspenders fallback for when the markers are absent (e.g. VT input not active on some consoles), `PasteBurstAccumulator` (`src/client/input.rs`) coalesces the marker-less burst: it buffers consecutive printable/`Enter`/`Tab` presses and, on the next idle poll or non-text key, emits one `ClientInputEvent::Paste` when the run is paste-shaped — ≥2 chars and contains `\n`, **or** ≥`LONG_RUN_PASTE_THRESHOLD` (24) chars with no newline at all (a long single-line paste, e.g. a URL or an unbroken paragraph, is still a zero-gap burst; without this it replayed as thousands of individual keys and risked dropping/truncating the text, issue #8); shorter runs and a lone Enter replay unchanged so normal typing still submits.

## Text selection (mouse + keyboard)

A drag selection in a pane stays highlighted after the mouse is released — `MouseEventKind::Up(MouseButton::Left)` in `src/app/input/mouse.rs` now just calls `Selection::finish()` (no auto-copy, no clear). The selected text is acted on by keyboard: **Ctrl+C** copies it (and keeps the highlight + shows the copy toast); **Delete/Backspace** deletes it from the pane and clears the highlight. Both are intercepted at the top of `prepare_terminal_key_forward` (`src/app/input/terminal.rs`) *before* the normal clear-on-keypress, guarded by `AppState::has_visible_selection()`. Copy sets `request_clipboard_write` via `copy_visible_selection`; delete sends one `0x7f` backspace per selected character via `delete_visible_selection_bytes` (count from `extract_selection`). The two terminal-key entry points drain the pending clipboard write through `forward_pending_clipboard_write` (same `ClipboardWrite` event path as copy mode). Double-click word copy is unchanged (still copies immediately with a brief highlight). Ctrl+C with no visible selection forwards the interrupt to the pane as before.
...
`snapshot_processes()` (`src/platform/windows.rs`) enumerates only pid/parent/name from the toolhelp snapshot; command lines are no longer read for every process on the system. `hydrate_command_lines(entries, pids)` fills `cmdline`/`argv`/`argv0` (via OpenProcess + NtQueryInformationProcess + ReadProcessMemory) for just the pids a caller inspects: `foreground_job` hydrates the pane shell + its descendants, `foreground_group_leader_job` hydrates one pid, and `session_processes` hydrates none. The detection loop calls these several times a second per pane, so this cuts per-tick OpenProcess/ReadProcessMemory work from every-process-on-the-system to a handful, eliminating the constant allocation churn behind the growing memory footprint. Unhydrated entries keep `argv0 = name` so pure selectors still work; `select_pane_foreground_job` only inspects hydrated shell/descendant entries.
...
Selection highlight background (`automatic_selection_bg` in `src/ui/panes.rs`) blends the host terminal background 55% toward the active `Palette::accent` (falling back to the panel background when the host reports no background, and to white/black when the accent is not an RGB color). The old 28% blend toward white left the highlight nearly invisible on dark themes such as Satisfactory.

The CWD fallback uses the *live* pane CWD: `render_statusline` takes the `TerminalRuntimeRegistry` and resolves `Tab::cwd_for_pane` + `derive_label_from_cwd`, so it shows the git repo root name of the foreground process (e.g. `herdr`) instead of the shell's spawn directory (`C:\GIT`).
...
**Resume uses the saved pane CWD (issue #13).** The #5/#9 precedence is inverted: restore spawns the resume shell in the saved pane CWD when it still exists, falls back to `project_cwd` only when it does not, and no longer walks up to the git repo root (`git_repo_root` is `pub(super)` again). The old order moved resumed panes out of the folder the user left them in, which the resumed agent then inherits as its CWD. `CommandBuilder::cwd` alone does not reliably place a restored pane, so restore types a `crate::pane::shell_cd_command` line into every restored shell, and an agent resume is queued in `App::deferred_pane_inputs` and typed 300 ms after that `cd` (`AGENT_RESUME_COMMAND_DELAY`). Do not put the `cd` and the resume on one line -- the agent then runs before the directory change takes effect. `App::flush_due_pane_inputs` drains the queue from all three loop drive points.
